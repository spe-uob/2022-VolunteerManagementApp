<template>
  <div class="filter_container">
    ...
    <div>
      <label for="id6">
        <input @click="setToggle3" class="filter-head-input" id="id6" type="checkbox">
        <div class="filter-head">
          <i class="arrow-right"></i>
          Organisation
        </div>
        <div v-show="toggle3" class="filter-body">
          <div>
            <label v-for="(organisation, index) in Organisations" :key="index">
              <input class="checkItem" type="checkbox" v-model="selectedValues1" :value="organisation">
              {{organisation}}
            </label>
          </div>
...
export default {
  ...
  async mounted() {
    ...
    this.Organisations = await Promise.all(response.map(async (result) => {
      return await this.getOrganisationById(result.referral_organisation);
    }));
  },
}
